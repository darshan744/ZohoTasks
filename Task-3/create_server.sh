...
set_master_server() {
    ...
    if ! $initdb -D $masterDirectory > $replicalog 2>&1;then
    ...
    if ! $pg_ctl -D $masterDirectory -o "-p $masterPort" start >> $replicalog 2>&1;then
        echo "Starting of master server failed"
        exit 1
    fi
    
    echo "[INFO] Creating a replication user"
    if ! $psql --port=$masterPort -c "create user repuser replication" postgres >> $replicalog 2>&1;then
    ...
    if ! $pg_ctl -D $masterDirectory reload >> $replicalog 2>&1;then
...
set_replicas() {
    ...
    for ((i=1 ; i<=$numberOfReplicas;i++));do
        ...
        if ! $pg_backup -h localhost -U repuser --checkpoint=fast -D $slaveDirectory -R --slot=$slotName -C --port=$masterPort >> $replicalog 2>&1;then
            echo "[ERROR] Configuring Standby server for dir : $slaveDirectory , port : $slavePort failed"
            continue
        fi

        if ! $pg_ctl -D $slaveDirectory -o "-p $slavePort" start >> $replicalog;then
